...
begin32:
	...
	mov al, 0b11111101
	out 0x21, al	;0xFC, unmask irq1 on PIC0
	mov al, 0xff
	out 0xa1, al	;0xFF, leave everything masked on PIC1
...
int_keyb:			; This interrupt is called when a button on the keybd is pressed
	pushad			; Save the registers, including the status word
	mov ax, 0x10	; We're going to set the descriptor/segment in gs to 0x10
	mov gs, ax		; We can't load immediate into gs, so go through ax

	in al, 0x64		; Read the keyboard port on the processor
	and al, 0x01	; Check the LSB. It's set when a key is pressed
	jz .keyb_buffer_empty
	
	;The buffer isn't empty. Let's map the scancode to ascii
	xor ebx, ebx
	in al, 0x60		; Read the actual scancode.
	mov bl, al
	test bl, bl
	jz .keyb_buffer_empty
	and al, 0x80
	...
	;Increment and save the cursor (have to add 2 because every other byte is a color pair)
	add dword [curpos], 2

	
.keyb_buffer_empty:

	; Signal that the interrupt is processed so others can use it.
	mov al, 0x20
	out 0x20, al
	
	popad		; Pop the flags including status register
	iret		; Return from interrupt
	
	curpos dd 0		; Points to where the next character is written.

; Just a stub interrupt for any interrupts we're not handling.
int_stub:
	iret
...
prot32modebad		db "Unable to enter 32b protected mode.", 0x0d, 0x0a, 0
